...
export default function getSupabaseAdmin(): SupabaseClient<Database> {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(
        'Intentando usar supabaseAdmin en el cliente. Esto es un riesgo de seguridad grave.'
      );
      console.error('El cliente supabaseAdmin debe usarse ÚNICAMENTE en el servidor.');
      console.error(
        'Revise su código para eliminar todas las importaciones de supabase-server en componentes cliente.'
      );
      console.error('=====================================');
    }
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    throw new Error(
      'Faltan las variables de entorno necesarias para crear el cliente Supabase Admin'
    );
  }
