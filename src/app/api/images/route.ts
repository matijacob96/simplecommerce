  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const { data: { publicUrl } } = supabaseAdmin.storage
        .from(BUCKET_NAME)
        .getPublicUrl(`productos/${productId}`);
      ...
      try {
        ...
        const { data, error } = await supabaseAdmin.storage
          ...
          .upload(`productos/${productId}`, imageBlob, {
            cacheControl: "604800",
            upsert: true, // Sobrescribe si ya existe
            contentType: contentType, // Especificar el tipo de contenido
          });
        ...
        return NextResponse.json(
          { error: error.message || "Error procesando la URL de la imagen" },
          { status: 500 }
        );
      }
    } 
    ...
    return NextResponse.json(
      { error: error.message || "Error en la API de imágenes" },
      { status: 500 }
    );
  }
}
