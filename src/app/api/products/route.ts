...
export async function POST(request: Request) {
  try {
    ...
    const { imageUrl: _, ...validProductData } = productData;
    ...
    if (validProductData.category_id !== undefined) {
      validProductData.category_id = validProductData.category_id === null ? null : Number(validProductData.category_id);
    }

    // Validaciones básicas
    if (!validProductData.name || typeof validProductData.name !== "string" || validProductData.name.trim() === "") {
      return NextResponse.json(
    ...
    if (validProductData.price === undefined || isNaN(parseFloat(validProductData.price)) || parseFloat(validProductData.price) < 0) {
      return NextResponse.json(
    ...
    if (validProductData.stock === undefined || isNaN(parseInt(validProductData.stock)) || parseInt(validProductData.stock) < 0) {
      return NextResponse.json(
