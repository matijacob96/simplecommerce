...
export default function NewSalePage() {
    ...
    const handleAddItem = () => {
        ...
        if (existingItemIndex >= 0) {
            ...
            if (product.stock < newQuantity) {
                message.warning(`No hay suficiente stock para agregar más unidades (disponible: ${product.stock})`);
                return;
            }
    ...
    const formatPrice = (price: number | null, exchangeRate?: number) => {
        ...
        if (exchangeRate) {
            ...
            return `U$ ${price.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} | AR$ ${priceARS.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        }
        
        // Si no tenemos tipo de cambio, mostrar solo USD
        return `U$ ${price.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    };
    ...
    const calculateTotal = () => {
        return saleItems.reduce((total, item) => {
            ...
            const itemPrice = item.selling_price || calculateSellingPrice(item.product);
            return total + (itemPrice * item.quantity);
        }, 0);
    };

    const renderSellingPrice = (item: NewSaleItem) => {
        const sellingPrice = item.selling_price || calculateSellingPrice(item.product);
                    ...
                    <Card title="Cliente" style={{ marginBottom: 24 }}>
                        ...
                        {showNewCustomerForm && (
                            <div style={{ padding: 16, background: '#f5f5f5', borderRadius: 4, marginTop: 16 }}>
                                <Title level={5} style={{ marginBottom: 16 }}>Nuevo Cliente</Title>
                                <Row gutter={[16, 16]}>
                                    <Col xs={24} md={12}>
                                    ...
                                    {products
                                        ...
                                        .map(product => (
                                            <Option key={product.id} value={product.id}>
                                                {product.name} - {formatUsdPrice(calculateSellingPrice(product))} (Stock: {product.stock})
                                            </Option>
                                        ))}
                                </Select>
                            </div>
                    ...
                    <Card title="Resumen de Venta" variant="outlined">
                        <div className="mb-4">
                            ...
                            <div className="flex justify-between">
                                ...
                                <Text strong>{formatPrice(calculateTotal(), exchangeRate || 1200)}</Text>
                            </div>
                        </div>
